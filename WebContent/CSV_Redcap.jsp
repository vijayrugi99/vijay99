...
else
{
	...
	newdate1 = dateend[1]+"/"+dateend[0]+"/"+dateend[2];
	dateto=newdate1;
	Calendar cal= Calendar.getInstance();
	cal.setTime(new Date(dateto));
	cal.add(Calendar.DATE, 1);
	Date endDate= cal.getTime();
	dateto = sdf.format(endDate);
...
//	MongoClient mongo = (MongoClient)com.kentropy.mongodb.MongoDAO.getMongoClient();
	...
	com.kentropy.mongodb.MongoDAO mongodao2 = new com.kentropy.mongodb.MongoDAO(db,homeborn);	
	  ...
	  if(res==1)
			{
			 ...
			 if(size1>0)
			 {
			        ...
			        int res1=new com.kentropy.mongodb.DeleteMongodbObject().insertCSVdetails(filename,datefrom,newdate1,db,csv,"redcap",df1.format(size1/1024));	
				    if(res1==3)
					{
